...
public abstract class HologramDisplay extends Thread {

    private final String hologramName;
    private Hologram hologram;
    private boolean isPlaced = false;

    public HologramDisplay(String hologramName) {
    ...
    public Location getLocation() {
        try {
            FileConfiguration config = HologramUtility.getPlugin().getConfig();

            return new Location(Bukkit.getWorld(getDefaultPath() + config.getString("world")),
    ...
    public void placeLeaderboard() {
        if(!isPlaced() && getLocation() != null) {
            hologram = HologramsAPI.createHologram(HologramUtility.getPlugin(), getLocation());
            isPlaced = true;
        }
    }
    ...
    public void updateLeaderboard() {
        Bukkit.getScheduler().scheduleSyncRepeatingTask(HologramUtility.getPlugin(), () -> {
            if(isPlaced) {
                hologram.clearLines();
                insertLines();
            }
